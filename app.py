...
def get_report_sheet():
    """Создаёт или возвращает лист для отчётов"""
    try:
        creds_dict = st.secrets["google"]
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        ...
        # Пытаемся получить лист "ОТЧЕТЫ", если нет — создаём
        try:
...
def generate_report(df, period_name):
    """Формирует отчёт в виде DataFrame с итогами"""
    if df.empty:
        return pd.DataFrame()
    ...
    abc_data["Доля"] = abc_data["Сумма"] / total_rev * 100
...
# === ОТПРАВКА ОТЧЁТА В GOOGLE SHEETS ===
def send_report_to_sheets(df, period_name):
    """Отправляет отчёт в отдельный лист Google Sheets"""
    ...
    try:
        ...
        # Сводка
        report_sheet.update_cell(5, 1, "1. СВОДКА")
        if not report_data["summary"].empty:
            summary_data = report_data["summary"].values.tolist()
            headers = report_data["summary"].columns.tolist()
            report_sheet.update([headers] + summary_data, value_input_option="USER_ENTERED")
        
        # Расходы по категориям
        row_offset = 5 + len(report_data["summary"]) + 2
        report_sheet.update_cell(row_offset, 1, "2. РАСХОДЫ ПО КАТЕГОРИЯМ")
        if not report_data["by_category"].empty:
            cat_data = report_data["by_category"].values.tolist()
            cat_headers = report_data["by_category"].columns.tolist()
            report_sheet.update([cat_headers] + cat_data, value_input_option="USER_ENTERED")
        
        # Топ-10 товаров
        row_offset = row_offset + len(report_data["by_category"]) + 2
        report_sheet.update_cell(row_offset, 1, "3. ТОП-10 ТОВАРОВ")
        if not report_data["top_products"].empty:
            top_data = report_data["top_products"].values.tolist()
            top_headers = report_data["top_products"].columns.tolist()
            report_sheet.update([top_headers] + top_data, value_input_option="USER_ENTERED")
        
        # Расходы по поставщикам
        row_offset = row_offset + len(report_data["top_products"]) + 2
        report_sheet.update_cell(row_offset, 1, "4. РАСХОДЫ ПО ПОСТАВЩИКАМ")
        if not report_data["by_supplier"].empty:
            sup_data = report_data["by_supplier"].values.tolist()
            sup_headers = report_data["by_supplier"].columns.tolist()
            report_sheet.update([sup_headers] + sup_data, value_input_option="USER_ENTERED")
        
        # ABC-анализ
        row_offset = row_offset + len(report_data["by_supplier"]) + 2
        report_sheet.update_cell(row_offset, 1, "5. ABC-АНАЛИЗ ТОВАРОВ")
        ...
        st.error(f"Ошибка отправки отчёта: {e}")
        return False
...
<style>
    ...
    .report-btn { background: linear-gradient(135deg, #2ecc71, #27ae60) !important; }
    .export-btn { background: linear-gradient(135deg, #3498db, #2980b9) !important; }
</style>
            ...
            if st.button("📊 Добавить данные"):
                for _, row in df_upload.iterrows():
                    new_row = pd.DataFrame([{
                        ...
                        "Товар": str(row.iloc[0]) if len(row) > 0 else "",
                        "Цена за ед.": float(row.iloc[1]) if len(row) > 1 else 0,
                        "Количество": float(row.iloc[2]) if len(row) > 2 else 1,
                        "Единица": str(row.iloc[3]) if len(row) > 3 else "шт",
                        "Сумма": float(row.iloc[4]) if len(row) > 4 else 0,
                        "Поставщик": "Замира",
                        "Категория": detect_category(str(row.iloc[0])) if len(row) > 0 else "Прочее",
                        "Примечание": ""
                    }])
...
if not filtered.empty:
    ...
    with col_r1:
        if st.button("📊 Сформировать отчёт", use_container_width=True):
            ...
            st.success("✅ Отчёт сформирован! Перейдите на вкладку '📈 Отчёты'")
    ...
    with col_r3:
        ...
        if not report_data["summary"].empty:
            ...
            st.download_button("📥 Скачать отчёт (Excel)", output.getvalue(), f"отчет_{period}_{datetime.now().strftime('%Y-%m-%d')}.xlsx", use_container_width=True)
...
st.caption("💾 Данные в Google Sheets | Отчёты можно выгрузить в Excel или Google Sheets")import streamlit as st
import pandas as pd
import plotly.express as px
from datetime import datetime, timedelta
import gspread
